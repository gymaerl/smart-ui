<template>
  <div class="header">
    <div class="left">
      ...
      <span style="font-size: 20px" onclick="state.isCollapse = !state.isCollapse">{{ name }}</span>
    </div>
    <div class="right">
      <el-popover
        placement="bottom"
...
export default {
  ...
  setup() {
    ...
    router.afterEach((to) => {
      ...
      state.name = localGet('path_map')[to.name]
      if (id && to.name == 'add') {
        state.name = '编辑商品'
      }
